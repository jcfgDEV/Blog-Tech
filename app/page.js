                ...
                <section className=" bg-white grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 p-2 ">
                    {blog.map((item, index) => (
                        <div className="relative" key={index}>
                           <div>
                           <a href={`/blog/${item.slug.split(" ").join("-")}`}><Image priority src={item.formated.img} width={1024} height={240} className="w-[1024px] h-[240px] object-cover rounded-md cursor-pointer s" alt="Cover Anime" /></a>
                           </div>
